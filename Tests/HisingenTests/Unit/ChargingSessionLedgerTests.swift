...
@Suite("Charging session lifecycle ledger")
struct ChargingSessionLedgerTests {
    private let vin = "ENGINE-TEST-VIN"
    private let start = Date(timeIntervalSince1970: 1_780_100_000)
    private let configuration = ChargingSessionLedgerConfiguration(
    ...
    func pauseResumeAndComplete() throws {
        ...
        ingest(ledger, minutes: 0, soc: 20, state: .charging, power: 11)
        let originalID = try #require(ledger.activeChargingSession(for: vin)?.id)
        ingest(ledger, minutes: 30, soc: 30, state: .charging, power: 11)
        ingest(ledger, minutes: 35, soc: 30, state: .paused, power: nil)
        #expect(ledger.activeChargingSession(for: vin)?.lifecycleState == .paused)
        ingest(ledger, minutes: 40, soc: 30, state: .scheduled, power: nil)
        ingest(ledger, minutes: 60, soc: 40, state: .charging, power: 11)
        #expect(ledger.activeChargingSession(for: vin)?.id == originalID)
        ingest(ledger, minutes: 90, soc: 50, state: .complete, power: nil)
    ...
    func stopDebounceAndResume() throws {
        let database = VehicleDatabase.inMemory()
        let ledger = database.charging
        ingest(ledger, minutes: 0, soc: 40, state: .charging, power: nil)
        let id = try #require(ledger.activeChargingSession(for: vin)?.id)
        ingest(ledger, minutes: 10, soc: 45, state: .idle, power: nil, connection: .disconnected)

        let pending = try #require(ledger.activeChargingSession(for: vin))
        ...
        ingest(ledger, minutes: 20, soc: 46, state: .charging, power: nil)
        #expect(ledger.activeChargingSession(for: vin)?.id == id)
        #expect(ledger.activeChargingSession(for: vin)?.pendingStopCount == 0)
        ingest(ledger, minutes: 30, soc: 50, state: .idle, power: nil, connection: .disconnected)
        ingest(ledger, minutes: 40, soc: 50, state: .idle, power: nil, connection: .disconnected)

        let completed = try #require(ledger.recentChargingSessions(for: vin).first)
    ...
    func integratedObservedPower() throws {
        ...
        for minute in stride(from: 0, through: 50, by: 10) {
            ingest(ledger, minutes: minute, soc: 20 + Double(minute) / 3,
                   state: .charging, power: 6)
        }
        ingest(ledger, minutes: 60, soc: 40, state: .complete, power: 6)
    ...
    func idleAtTargetCompletes() throws {
        let database = VehicleDatabase.inMemory()
        let ledger = database.charging
        ingest(ledger, minutes: 0, soc: 70, state: .charging, power: nil)
        ingest(ledger, minutes: 30, soc: 80, state: .idle, power: nil)
        ingest(ledger, minutes: 31, soc: 80, state: .idle, power: nil)
    ...
    func faultInterruption() throws {
        let database = VehicleDatabase.inMemory()
        let ledger = database.charging
        ingest(ledger, minutes: 0, soc: 20, state: .charging, power: 7)
        ingest(ledger, minutes: 15, soc: 25, state: .idle, power: nil, connection: .fault)
    ...
    func staleSessionBoundary() throws {
        ...
        ingest(ledger, minutes: 49 * 60, soc: 55, state: .charging, power: nil)

        let newSession = try #require(ledger.activeChargingSession(for: vin))
    ...
    func legacySchemaMigratesWithoutLaunchFailure() throws {
        ...
        let migrated = try #require(database.charging.recentChargingSessions(for: "LEGACY-VIN").first)
    ...
    private func ingest(
        _ ledger: ChargingSessionLedger, minutes: Int, soc: Double,
