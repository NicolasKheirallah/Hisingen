...
struct LaunchAtLoginReconciliationTests {
    ...
    func intentOnRegistersWhenNotYetRegistered() {
        #expect(resolve(intent: true, status: .notRegistered) == .register)
    }

    @Test
    func intentOnWithLiveRegistrationIsANoOp() {
        #expect(resolve(intent: true, status: .enabled) == .none)
    }

    @Test
    func approvalIsOnlyChasedWhenTheUserJustAskedForIt() {
        // Automatic startup reconcile: don't yank the user into System Settings.
        #expect(resolve(intent: true, status: .requiresApproval, userInitiated: false) == .none)
        // They just flipped the switch: sending them to approve is expected.
        #expect(resolve(intent: true, status: .requiresApproval, userInitiated: true) == .promptForApproval)
    }

    @Test
    func intentOffUnregistersAnActiveOrPendingLoginItem() {
        #expect(resolve(intent: false, status: .requiresApproval) == .unregister)
    }
        ...
        for intent in [true, false] {
            for status in states {
                for userInitiated in [true, false] {
                    ...
                    #expect(action != .restoreClearedIntent || intent == false, "restore should only apply when the stored intent was off")
                }
            }
        }
    }
