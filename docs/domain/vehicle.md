...
- `cacheableCopy` — the version persisted to `VehicleStateStore`. Built by passing only a specific subset of fields (VIN, current battery/charging/range, availability, model name/year, powertrain/fuel, capability observations, and timestamps) into the initializer — completed charging history lives only in SQLite, while every other omitted field, including PII *and* location/exterior/lock status, is dropped by omission rather than an explicit strip list. See [architecture/persistence.md](../architecture/persistence.md).
