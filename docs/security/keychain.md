...
**This does not match the public-facing docs.** README.md and TERMS.md both state Keychain items use `WhenUnlockedThisDeviceOnly`. The actual accessibility level, `AfterFirstUnlockThisDeviceOnly`, is weaker in one specific way: it only requires the device to have been unlocked *once since the last restart*, not that it be unlocked *at the moment of access* — meaning a background refresh could technically read these items while the screen is locked (as long as the Mac has been unlocked since boot). Both levels are `ThisDeviceOnly` (excluded from iCloud Keychain sync, never leaves the device via Apple's sync), which is the property both the docs and the code agree on. This is flagged here as a documentation-accuracy issue, not a currently-exploited vulnerability — see [architecture/technical-debt.md](../architecture/technical-debt.md) for the recommended fix direction.
