...
1. **Compiled into every build, disabled by default.** The former `HISINGEN_EXPERIMENTAL_REMOTE` compile flag was removed in [ADR-0009](../adr/0009-remote-commands-compiled-into-all-builds.md) — a release binary now does contain dispatch code. What stands between that code and a vehicle is runtime: no remote `AppFeature` is in `FeatureSelection.default`, so each must be switched on in Settings, and `AppDelegate.performRemoteCommand` refuses any command whose feature is off.
