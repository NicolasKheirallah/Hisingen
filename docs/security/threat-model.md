...
| Remote-command receipts | Bounded per-VIN `UserDefaults` collection, separate from telemetry | Reveals recent command intent and outcome |
...
1. **Compiled into every build, disabled by default.** The former `HISINGEN_EXPERIMENTAL_REMOTE` compile flag was removed in [ADR-0009](../adr/0009-remote-commands-compiled-into-all-builds.md). A release binary contains dispatch code, but no remote `AppFeature` is in `FeatureSelection.default`; each must be switched on in Settings, and `CommandCoordinator` refuses any command whose feature is off.
2. **Only software installation is actually reachable for Polestar today.** `ota_mobcache.SchedulerService` accepts `Schedule`/`InstallNow`/`CancelSchedule` from the mobile-app OIDC client Hisingen now uses. Locks, windows, climate, and charging remain hardcoded `.disabled(true)` in `ControlsTabView`, and their PCCS request paths are unverified — see [architecture/technical-debt.md](../architecture/technical-debt.md).
...
6. **A backend acknowledgment is never treated as confirmed execution.** `RemoteCommandOutcome` distinguishes `.accepted`, `.delivered`, and `.completed`; `RefreshCoordinator` requires fresh matching telemetry before marking an observable command confirmed. Optimistic values remain display-only and TERMS.md is explicit that Hisingen does not guarantee physical execution.
