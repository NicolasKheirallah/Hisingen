...
- **Read-only by default.** Standard builds never send a vehicle command. Experimental remote control exists behind a `HISINGEN_EXPERIMENTAL_REMOTE` compile flag that is never set in CI or release builds. See [domain/vehicle.md](domain/vehicle.md) and [security/threat-model.md](security/threat-model.md).
...
→ api/polestar.md or api/volvo.md
```
