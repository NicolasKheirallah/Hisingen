...
When investigating a user report, ask for a diagnostic bundle: **Settings → SQLite Storage & Data → Export Diagnostic Logs**. The resulting JSON (`schemaVersion: 3`) contains: `meta` (app version/build, macOS version, hardware model, locale/timezone), `unifiedLog` (the last 24 hours of Hisingen's own unified-log entries), `apiRequests` (redacted request history from `APIDiagnosticLogStore`, including status codes, durations, gRPC statuses, and payloads), `refreshDiagnostics` (refresh state, live-stream metrics, and active command-confirmation state), `polestarVDMS` when metadata discovery is in persistent-failure backoff, `commandAudit`, and `databaseStats`.
...
currently report charging. It never activates Hisingen or steals focus; drag it anywhere and the
