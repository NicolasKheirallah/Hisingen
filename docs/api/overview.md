...
| Polestar remote-command dispatch | **Experimental / unverified** | Gated behind `HISINGEN_EXPERIMENTAL_REMOTE`, never enabled in CI or release builds; the backend is expected to reject unpaired clients (`grpc-status 16`) even when the flag is set |
