...
struct VehicleFleetSummary {
    ...
    init(states: [VehicleState], now: Date = Date()) {
        ...
        let odometers = states.filter { $0.hasFreshReading(.odometer, now: now) }.compactMap(\.odometerKm)
        ...
        let known = states.filter {
            ...
            if case .unknown = $0.chargingState { return false }
            return true
        }
        let charging = known.filter(\.isCharging)
        chargingCount = charging.count
        chargingPowerWatts = charging.compactMap(\.chargingPowerWatts).filter { $0 > 0 }.reduce(0, +)
